...
const Calendar = () => {
  ...
  if (clickedDate >= today) {
    ...
    // Ažuriranje stanja rezervacija iz backend-a
    await fetchAvailableTimes();

    const reservedTimes = reservedLessons
      .filter((lesson) => lesson.start.startsWith(info.dateStr))
      .map((lesson) => new Date(lesson.start).toTimeString().slice(0, 5));

    const availableTimesForDate = allTimes.filter(
      (time) => !reservedTimes.includes(time)
    );

    if (clickedDate.getTime() === today.getTime()) {
      const now = new Date();
      const filteredTimes = availableTimesForDate.filter((time) => {
    ...
    if (selectedDate && selectedTime && user.id !== teacherId) {
      ...
      try {
        ...
        if (response.status === 200 || response.status === 201) {
          ...
          // Ažuriranje stanja rezerviranih termina
          await fetchAvailableTimes();
  
          // Uklanjanje potvrđenog termina iz slobodnih termina
